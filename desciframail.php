...
/*
Basic program to encrypt-decrypt messages
Denobisipsis 2015
Compile with Bambalam
*/
...
* @author Matt Harris
**/
  function encrypt($plaintext, $key, $mode=10, $padding=21, $iv=NULL) {
    if ( $mode == 11 and empty($iv) ) {
...
* @author Matt Harris
**/
  function decrypt($ciphertext, $key, $mode=10, $padding=21, $iv=NULL) {
...
Uso: desciframail D/E \"mensaje\" \"clave\"\r\n

Ejemplos:

Encriptar:    desciframail E \"mensajedeprueba\" \"clavemuydura\"

Desencriptar: desciframail D \"mensajedepruebacifrado\" \"clavemuydura\"
...
";
	...
	if ($mode=="D")
		{
	  $mail	=base64_decode(gzuncompress(pack("H*",$mail)));
		
		$decrypt    	=new Blowfish($key,10);
		echo $decrypt	=$decrypt->decrypt($mail,$key,10);
		}
	else
		{
		$encrypt    	=new Blowfish($key,10);
		$encrypt	=$encrypt->encrypt($mail,$key,10);	
	  echo $mail	=bin2hex(gzcompress(base64_encode($encrypt)));
		}							
?>
